...
export const applyToJob = (req, res) => {
  ...
  res.status(201).json({ id, userId, jobId, coverLetter: coverLetter || null });
};
...
export const getApplications = (req, res) => {
  ...
  if (role === "admin") {
    const rows = db.prepare(`
      SELECT a.id, a.userId, u.name as userName, a.jobId, j.title as jobTitle, a.coverLetter, a.createdAt
      FROM applications a
